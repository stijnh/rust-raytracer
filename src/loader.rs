...
use json::{parse as json_parse, Error as JsonError, JsonValue};
...
pub fn load_obj(filename: &str) -> Result<Vec<(Vec3D, Vec3D, Vec3D)>, LoaderError> {
    ...
    for (lineno, line) in BufReader::new(file).lines().enumerate() {
        ...
        match token {
            ...
            "f" => {
                ...
                if let (Some(i), Some(j), Some(k)) = (is, js, ks) {
                    if min!(i, j, k) == 0 || max!(i, j, k) < vertices.len() + 1 {
                        triangles.push((vertices[i - 1], vertices[j - 1], vertices[k - 1]));
                    } else {
                        return parse_error(
                            format!(
            ...
            "vn" => { /* TODO */ }
            _ => {
                return parse_error(format!("unexpected token '{}'", token), lineno);
            }
        }
    }
...
pub fn load_scene(filename: &str) -> Result<(Camera, Box<dyn Geometry>), LoaderError> {
    ...
    let parse_error =
        |msg, path| LoaderError::ParseError(format!("{}: in file {} for {}", msg, filename, path));
    ...
    let parse_vec3d = |val: &JsonValue, path| {
        let (x, y, z) = if val.is_array() {
            ...
            raise!(parse_error(
                format!("element must be array or object, not {}", val),
                path
            ));
        } else {
        ...
        match (x.as_f32(), y.as_f32(), z.as_f32()) {
            (Some(x), Some(y), Some(z)) => Ok(vec3d(x, y, z)),
            _ => {
                raise!(parse_error(
                    format!("failed to parse ({}, {}, {}) as 3D vector", x, y, z),
                    path
                ));
            }
        }
    ...
    let cam = Camera::new().position(cam_pos).look_at(cam_lookat, cam_up);
